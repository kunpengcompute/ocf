...
/*
 * Callback function called when lookup completes.
 */
void complete_lookup(struct ocf_io *io, int error)
{
	struct volume_data *data = ocf_io_get_data(io);

	printf("LOOKUP COMPLETE (error: %d)\n", error);
...
/*
 * Callback function called when invalid completes.
 */
void complete_invalid(struct ocf_io *io, int error)
{
	struct volume_data *data = ocf_io_get_data(io);

	printf("INVALID COMPLETE (error: %d)\n", error);
...
{
	struct volume_data *data1, *data2, *data3, *data4, *data5;

	/* Allocate data buffer and fill it with example data */
	data1 = ut_ctx_data_alloc(1);
	if (!data1)
	...
	/* Allocate data buffer for lookup */
	data3 = ut_ctx_data_alloc(1);
	if (!data3)
		error("Unable to allocate data3\n");
	/* Prepare and submit lookup IO */
	submit_io(core, data3, 0, 4096, OCF_LOOKUP, complete_lookup);
	/* After lookup completes, complete_lookup() callback will be called,
	 * where we print miss or hit.
	 */

	usleep(10);

	/* Allocate data buffer for invalid */
	data4 = ut_ctx_data_alloc(1);
	if (!data4)
		error("Unable to allocate data4\n");
	/* Prepare and submit invalid IO */
	submit_io(core, data4, 0, 4096, OCF_INVALID, complete_invalid);
	/* After invalid completes, complete_invalid() callback will be called. */

	usleep(10);

	/* Allocate data buffer for lookup */
	data5 = ut_ctx_data_alloc(1);
	if (!data5)
		error("Unable to allocate data5\n");
	/* Prepare and submit lookup IO */
	submit_io(core, data5, 0, 4096, OCF_LOOKUP, complete_lookup);
	/* After lookup completes, complete_lookup() callback will be called,
	 * where we print miss or hit.
	 */
}
...
static void engine_uc_test01(void **state)
{
	struct cache_priv *cache_priv;
	struct simple_context context;
	ocf_ctx_t ctx;
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(engine_uc_test01),
    };
